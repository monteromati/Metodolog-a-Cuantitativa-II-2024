...
# Para llevar a cabo este ejercicio, tenemos que crear una variable que represente el porcentaje de mujeres respecto del total de academicoss
personal_2021$por_muj <- round((personal_2021$total_mujeres/personal_2021$total_general)*100)

# 1. Análisis descriptivo univariado
# Dado que se trata de una variable continua, usamos los descriptivos adecuados
summary(personal_2021$por_muj)
sd(personal_2021$por_muj); var(personal_2021$por_muj)

# Histograma y densidad: para conocer la distribución de los datos
# Histograma
ggplot(personal_2021, aes(x=por_muj))+
  geom_histogram()
# Densidad
ggplot(personal_2021, aes(x=por_muj))+
  geom_density()
...
ggplot(personal_2021, aes(x=por_muj))+
  geom_histogram(aes(y = ..density..))+
  geom_density()
# Boxplot: para detectar casos extremos
ggplot(personal_2021, aes(y=por_muj, x=""))+
  geom_boxplot()

# 2. Análisis descriptivo bivariado
# Estadísticos básicos por grupo de interés: tipo_ies
by(personal_2021$por_muj, personal_2021$tipo_ies_n, summary)

### Graficamos cómo difiere el porcentaje de mujeres según tipo de institución
# Gráfico de barras (con estadísticos descriptivos)
ggplot(personal_2021, aes(x=tipo_ies_n, y=por_muj)) +
  geom_bar(stat = "summary", fun = "mean")
# Histograma
ggplot(personal_2021, aes(x = por_muj, fill = tipo_ies_n)) + 
  geom_histogram()
# Densidad
ggplot(personal_2021, aes(x=por_muj, fill = tipo_ies_n))+
  geom_density()
# Boxplot
ggplot(personal_2021, aes(y=por_muj, x="", fill = tipo_ies_n))+
  geom_boxplot()
